...
for(i in 1:n) {
  ...
  dtrain <- xgb.DMatrix(data = as.matrix(train_data), label = train_data$DEATH_EVENT)

  dtest = xgb.DMatrix(data = as.matrix(test_data), label = test_data$DEATH_EVENT)
...
```{r}
write.csv(final_result, "xgboost_classifier_top_features_results.csv")
```

```{r}
write.csv(results_df, "xgboost_classifier_top_features_results_log.csv")
```